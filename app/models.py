...
class Token(SQLModel):
    """Schema de respuesta para el login y el refresh (JWT)"""
    access_token: str
    refresh_token: Optional[str] = None
    token_type: str = "bearer"

class TokenData(SQLModel):
    """Schema para el payload del JWT (para dependencias)"""
...
class User(SQLModel, table=True):
    ...
    capture_streak: Optional[int] = Field(default=0)  # <-- NUEVO CAMPO
...
class TeamUpdate(SQLModel):
    ...
    description: Optional[str] = None
    #Esto es el payload de los IDs a actualizar
