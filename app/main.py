...
logging.basicConfig(level=logging.INFO,format='%(asctime)s %(name)s %(levelname)s %(message)s',
    handlers=[logging.FileHandler('pokedex_api.log'),logging.StreamHandler()]) #Log a consola
...
async def lifespan(app: FastAPI):
    ...
    # Solo crea tablas si NO estamos en modo testing
...
app = FastAPI( title="Pokédex Personal API",version="v1.0.0",
...
app.add_middleware(CORSMiddleware,allow_origins=origins,allow_credentials=True,
    allow_methods=["GET", "POST", "PUT", "PATCH", "DELETE"],  #Métodos especificados
    allow_headers=["Authorization", "Content-Type"],  #Headers especificados
    max_age=3600,
)
...
async def log_requests(request: Request, call_next):
    ...
    #Logs requeridos: Peticiones HTTP (metodo, path)
...
#CONFIGURACIÓN DE RUTAS Y VERSIONADO
...
#La v2 no tiene rutas funcionales aún, pero su estructura está lista.
